...
export default class Help extends Component<Props> {
  ...
  componentDidMount() {
    Events.on('receiveResponse', 'receiveResponseHelp', this.onReceiveResponse.bind(this))
    Events.on('updateUI', 'updateUIHelp', this.onUpdateUI.bind(this))
  ...
  onReceiveResponse (responceData) {
    ...
    if (responceData.methodName == 'CMSData') {
        ...
        if (responceData.Status == true) {
          console.log('responceData.Results.CmsData_List[0].Description:=',responceData.Results.CmsData_List[0].Discription)
          this.setState({
            helpResponse:responceData.Results.CmsData_List[0].Discription
          })
        }
        else{
    ...
    return (
      ...
      {/* HeaderView */}
        ...
        <WebView style={{
            ...
            padding:20,
          }} source={{html: this.state.helpResponse}}
