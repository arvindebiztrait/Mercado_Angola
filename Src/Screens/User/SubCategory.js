...
export default class SubCategory extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
        dataSource:ds.cloneWithRows(this.props.navigation.state.params.subCategory),
        searchText:'',
        arrCategory: Array.from(new Set(this.props.navigation.state.params.subCategory)),  //this.props.navigation.state.params.subCategory,//this.getStaticData(),
        distance:0,
        minValue:0,
        maxValue:100,
        ...
        categoryId: this.props.navigation.state.params.isFromHome === true ? this.props.navigation.state.params.data.CategoryID : this.props.navigation.state.params.categoryId,
        latitude: this.props.navigation.state.params.isFromHome === true ? this.props.navigation.state.params.latitude : 0.0,
        longitude: this.props.navigation.state.params.isFromHome === true ? this.props.navigation.state.params.longitude : 0.0,
    };
  }
 ...
 onValueChangeCheckBox(rowdata,checked){
    ...
    // arrInd =  Object.assign([...arrInd], {index: rowdata})
  ...
  onClickApplyFilter() {
    ...
    for (var i=0; i <this.state.arrCategory.length; i++) {
      if (this.state.arrCategory[i].checkStatus == true) {
        ...
        arrSelectedCatName.push(this.state.arrCategory[i].CategoryName)
      }
    }
    ...
    var data = {
      ...
      categoryName:arrSelectedCatName.toString()
    }
    Events.trigger("selectedCategory",data)
    if (this.props.navigation.state.params.isFromHome) {
      this.props.navigation.navigate('professionalList',{isFromHome:true,categoryId:arrSelectedCat.toString(),data:this.state.categoryData,latitude:this.state.latitude,longitude:this.state.longitude})
    }
    else {
