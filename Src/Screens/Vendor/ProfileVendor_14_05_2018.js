...
export default class ProfileVendor extends Component<Props> {
  ...
  componentDidMount() {
    ...
    Events.on('selectedCategory', 'selectedCategoryProfileVendor', this.onSelectedCategory.bind(this)) 
    Events.on('updateUI', 'updateUIProfileVendor', this.onUpdateUI.bind(this))
  ...
  getSelectedIndustry(arrSelectedCat) {
    ...
    if (arrSelectedCat.length > 0) {
      ...
      var arrName = []
      var arrIds = []
      for (i=0;i<arrSelectedCat.length;i++) {
        arrName.push(arrSelectedCat[i].CategoryName)
        arrIds.push(arrSelectedCat[i].CategoryID)
      }
      this.setState({
        industry:arrName.toString(),
        industryIds:arrIds.toString()
      })
    }
  }
        ...
        <View style={{
          ...
          <View style={{
            ...
            <View pointerEvents={this.state.isEditable ? 'auto' : 'none'} style={{
              ...
              <Text style={{width:'90%'}} numberOfLines={1}>{this.state.industry ? this.state.industry : LS.LString.industryPlaceHolderText}</Text>
              <Image style={{
                  // backgroundColor:'yellow',
                  flex:1,
                  width:'10%',
